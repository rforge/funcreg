...
####################################################
fdaCV <- function(y, t, lamvec, kvec, L=2, rangeval = c(0,1),
...
## cross-validation. The linear Case
 #################################################
getLam <- function(y, t, lamInt, k, L=2, rangeval = c(0,1),
                   create_basis=create.bspline.basis, 
...
### The linear case
#########################################################################
getLandKopt <- function(y, t, lamInt, kvec, L=2, rangeval=c(0,1),
                        create_basis=create.bspline.basis, 
                        maxitalgo=100, tolalgo=1e-6, type=c("Brent", "GS"), ...)
    {
        ...
        lambda <- res[1,w]
        k=kvec[w]
        obj <- coefEst(y=y, t=t, lam=lambda, k=k, L=L, rangeval = rangeval,
                       create_basis=create_basis, ...)
        obj$cv <- res[4,w]
...
#####################################
getLandK <- function(y, t, lamvec, kvec, L=2, rangeval=c(0,1),
                     create_basis=create.bspline.basis, ...)
    {
        cv <- fdaCV(y, t, lamvec, kvec, L, rangeval, create_basis, ...)
        w <- which(cv==min(cv,na.rm=TRUE), arr.ind=TRUE)
        lambda <- lamvec[w[1]]
        k=kvec[w[2]]
        res <- coefEst(y=y, t=t, lam=lambda, k=k, L=L, rangeval = rangeval,
                         create_basis=create_basis, ...)
